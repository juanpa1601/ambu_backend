...
class ListAmbulancesApplicationService:
    ...
    def list_ambulances(
        ...
        try:
            ...
            ambulances = Ambulance.objects.filter(is_active=True).order_by('mobile_number')
            ...
            ambulances_data: list[dict[str, Any]] = [
                {
                    ...
                    'display_name': f"Móvil {ambulance.mobile_number} - {ambulance.license_plate}" if ambulance.license_plate else f"Móvil {ambulance.mobile_number}"
                }
